...
interface AppContextType {
  ...
  setDownloadWidth: React.Dispatch<React.SetStateAction<number>>;
  downloadHeight: number;
  setDownloadHeight: React.Dispatch<React.SetStateAction<number>>;
  downloadUnit: "px" | "in" | "cm" | "mm";
  setDownloadUnit: React.Dispatch<
    React.SetStateAction<"px" | "in" | "cm" | "mm">
  >;
}
      ...
      <div className="space-y-6">
        {[
          ...
          },
          {
            ...
            a: "In the 'Download' card, select your desired quality, size, units, and format, then click the 'Download' button. If you've added a text label, it will be included in PNG, JPEG, and WEBP downloads.",
          },
          {
...
const App: React.FC = () => {
  const getInitialState = () => {
    const defaultState = {
      ...
      downloadMode: "simple" as "simple" | "advanced",
      downloadWidth: 1000,
      downloadHeight: 1000,
      downloadUnit: "px" as "px" | "in" | "cm" | "mm",
      colorType: "single" as "single" | "gradient",
      gradient: {
        type: "linear",
        rotation: 0,
        colorStops: [
    ...
    try {
      ...
      if (savedStateJSON) {
        return { ...defaultState, ...JSON.parse(savedStateJSON) };
      }
    } catch (error) {
  ...
  const [qrOptions, setQrOptions] = useState<QRCodeStylingOptions>(initialState.qrOptions);
  const [logo, setLogo] = useState<string | null>(initialState.logo);
  const [textLabel, setTextLabel] = useState(initialState.textLabel);
  const [textColor, setTextColor] = useState(initialState.textColor);
  const [textMargin, setTextMargin] = useState(initialState.textMargin);
  const [textFontSize, setTextFontSize] = useState(initialState.textFontSize);
  const [textFontFamily, setTextFontFamily] = useState(initialState.textFontFamily);
  const [fontWeight, setFontWeight] = useState(initialState.fontWeight);
  const [isTextItalic, setIsTextItalic] = useState(initialState.isTextItalic);
  const [textBackgroundColor, setTextBackgroundColor] = useState<string | null>(initialState.textBackgroundColor);
  const [downloadMode, setDownloadMode] = useState<'simple' | 'advanced'>(initialState.downloadMode);
  const [downloadWidth, setDownloadWidth] = useState(initialState.downloadWidth);
  const [downloadHeight, setDownloadHeight] = useState(initialState.downloadHeight);
  ...
  useEffect(() => {
    ...
    isMounted, qrOptions, data, logo, textLabel, textColor, textMargin, textFontSize,
    textFontFamily, fontWeight, isTextItalic, textBackgroundColor, downloadMode,
